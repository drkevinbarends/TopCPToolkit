...
`maxEta`
:   the maximum electron $\vert\eta\vert$ (float). The default is 2.47.
...
:   whether to decorate the calo-cluster $\eta$. The default is `False`.
...
!!! success "Additional variables toggled by `decorateTruth`:"
    ...
    - `truth_firstEgMotherPdgId`: PDG ID of the mother truth particle (no systematics)
    - `truth_firstEgMotherTruthOrigin`: origin of the mother truth particle (no systematics)
    - `truth_firstEgMotherTruthType`: type of the mother truth particle (no sytematics)

!!! success "Additional variables toggled by `decorateCaloClusterEta`:"
    - `caloEta2`: the calo-cluste $\eta$ from the 2nd sampling layer (no systematics)
...
`identificationWP`
:   the ID WP (string) to use. Supported ID WPs: `TightLH`, `MediumLH`, `LooseBLayerLH`, `TightDNN`, `MediumDNN`, `LooseDNN`, `TightNoCFDNN`, `MediumNoCFDNN`, `VeryLooseNoCF97DNN`.
...
:   whether to retain only electrons satisfying the working point requirements. The default is `True`.
