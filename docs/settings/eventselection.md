...
:   whether to use dressed truth electron and truth muon kinematics rather than simple P4 kinematics. The default is `True`.
...
| `SUM_EL_N_MU_N_TAU_N` | `ptmin $ ref`<br><br>`ptminEL ptminMU ptminTAU $ ref` | Compares (`$`) the number of electrons, muons and taus with <br>$p_\mathrm{T}>$`ptmin` (in MeV) to `ref`.<br>Same, but with different cuts for electrons/muons/taus. |
...
| `RUN_NUMBER` | `$ ref` | Compares (`$`) the run number (random, if MC) to `ref`.|
