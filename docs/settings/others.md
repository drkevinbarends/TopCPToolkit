...
### [HSTPFilterBlock](https://gitlab.cern.ch/atlasphys-top/reco/TopCPToolkit/-/blob/main/source/TopCPToolkit/python/HSTPFilterConfig.py)
Name in YAML: **HSTPFilter**

**Only applicable to dijet MC samples and implemented for DSIDs 364700-364712.** The algorithm removes events that have a leading in-time truth PU jet with $p_{T}$ greater than that of the leading truth HS jet. This is intended to perform a proper dijet normalization compared to the legacy "MC Cleaning Cuts". For more details see the relevant [twiki](https://twiki.cern.ch/twiki/bin/view/AtlasProtected/JetEtMissMCSamples#Dijet_normalization_procedure_HS) from JETM.

`truthHSCollection`
:   the truth HS jet collection to be used. Default is `"AntiKt4TruthDressedWZJets"` since the nominal recommendation from JETM (`"AntiKt4TruthJets"`) is not available in DAOD_PHYS. The differences are expected to be small.

`truthPUCollection`
:   the truth PU jet collection to be used. Default is `"InTimeAntiKt4TruthJets"`.
...
!!! success "Registers the following variables:"
    ...
    - `weight_pileup`: the weight for pileup reweighting
    - `weight_beamspot`: the weight to correct the beamspot size in Run 2 (no systematics)
    - `averageInteractionsPerCrossing`: the $\langle\mu\rangle$ per bunch crossing
    - `actualInteractionsPerCrossing`: the actual $\mu$ per bunch crossing
