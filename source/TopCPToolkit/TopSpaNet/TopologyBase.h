...
class TopSpaNetTopology : public ONNXWrapper {
 public:
  TopSpaNetTopology(const std::string& name, const std::string& model_even,
                    const std::string& model_odd);

  virtual StatusCode execute() {
    return StatusCode::SUCCESS;
  };  // we would need to refactor things a bit in order to let the tool know
      // about the selected objects, then we could use execute() instead of
      // Predict()

  virtual void Predict(ConstDataVector<xAOD::ElectronContainer>& electrons,
                       ConstDataVector<xAOD::MuonContainer>& muons,
                       ConstDataVector<xAOD::JetContainer>& jets, float met_met,
                       float met_phi, unsigned long long eventNumber) = 0;
