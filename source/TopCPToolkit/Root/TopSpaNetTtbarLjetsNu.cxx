...
TopSpaNetTtbarLjetsNu::TopSpaNetTtbarLjetsNu(const std::string& name,
                                             const std::string& model_even,
                                             const std::string& model_odd)
...
void TopSpaNetTtbarLjetsNu::Predict(
    ConstDataVector<xAOD::ElectronContainer>& electrons,
    ConstDataVector<xAOD::MuonContainer>& muons,
    ConstDataVector<xAOD::JetContainer>& jets, float met_met, float met_phi,
  ...
  if (leptons.size() > 1)
    ANA_MSG_VERBOSE("WARNING: Multiple leptons found, using first one only");

  // currently theres a bug with spanet whereby we need to use a batchsize > 1;
  // so for now, we will just add a dummy second event, hence the [2] below
  ...
  for (long unsigned int i = 0; i < static_cast<long unsigned int>(m_MAX_JETS);
       ++i) {
    std::vector<float> jet_kin;

    if (i < jets.size()) {
      // TODO: Avoid hard coding these? maybe give the list of inputs in the
      // config, or read the spanet config file, or something?
      auto jet = jets[i];
      ...
      if (acc_btagging_quantile.isAvailable(*jet)) {
        jet_kin.push_back(acc_btagging_quantile(*jet));
      } else {
        jet_kin.push_back(0);
      }
      jet_masks[0][i] = 1;

      for (int j = 0; j < m_NUM_JET_FEATURES; ++j)
        jet_values[0][i][j] = jet_kin[j];
    ...
    jet_masks[1][i] = 0;
    for (int j = 0; j < m_NUM_JET_FEATURES; ++j)
      jet_values[1][i][j] = 0.0;
  }

  for (long unsigned int i = 0;
       i < static_cast<long unsigned int>(m_MAX_LEPTONS); ++i) {
    ...
    if (i < leptons.size()) {
      ...
      for (int j = 0; j < m_NUM_LEPTON_FEATURES; ++j)
        lepton_values[0][i][j] = lepton_kin[j];
  ...
  for (int i = 0; i < m_MAX_GLOBALS; ++i) {
    ...
    for (int j = 0; j < m_NUM_GLOBAL_FEATURES; ++j)
      global_values[0][i][j] = global_kin[j];
    ...
    global_masks[1][i] = 0;
    for (int j = 0; j < m_NUM_GLOBAL_FEATURES; ++j)
      global_values[1][i][j] = 0.0;
  }

  // create a vector of 64-bit integers with the input tensor dimension
  std::vector<int64_t> input_jet_tensor_dims = {
      2, m_MAX_JETS,
      m_NUM_JET_FEATURES};  // TODO: Fix this when the batching is sorted out
  std::vector<int64_t> input_lepton_tensor_dims = {
      2, m_MAX_LEPTONS,
      m_NUM_LEPTON_FEATURES};  // TODO: Fix this when the batching is sorted out
  std::vector<int64_t> input_global_tensor_dims = {
      2, m_MAX_GLOBALS,
      m_NUM_GLOBAL_FEATURES};  // TODO: Fix this when the batching is sorted out
  // and the masks
  ...
  this->linkInputs(**jet_values, jet_input_size, input_jet_tensor_dims.data(),
                   input_jet_tensor_dims.size());
  int jet_mask_size = 1;
  for (unsigned long int i = 0; i < input_jet_tensor_dims_mask.size(); ++i) {
    jet_mask_size = jet_mask_size * input_jet_tensor_dims_mask[i];
  }
  this->linkInputs(*jet_masks, jet_mask_size, input_jet_tensor_dims_mask.data(),
                   input_jet_tensor_dims_mask.size());
  ...
  this->linkInputs(**lepton_values, lepton_input_size,
                   input_lepton_tensor_dims.data(),
                   input_lepton_tensor_dims.size());
  ...
  this->linkInputs(*lepton_masks, lepton_mask_size,
                   input_lepton_tensor_dims_mask.data(),
                   input_lepton_tensor_dims_mask.size());
  ...
  this->linkInputs(**global_values, global_input_size,
                   input_global_tensor_dims.data(),
                   input_global_tensor_dims.size());
  ...
  this->linkInputs(*global_masks, global_mask_size,
                   input_global_tensor_dims_mask.data(),
                   input_global_tensor_dims_mask.size());
  ...
  // now read the output
  auto thpred = this->getOutputs<float>(
      "th_assignment_probability");  // TODO: Might be nice to avoid hard coding
                                     // this?
  float max = -999;
  ...
  int dim =
      m_MAX_JETS +
      m_MAX_LEPTONS;  // Globals cannot be predicted so are not included here

  // hadtop prediction order in config is bhad, up, down
  for (int row = 0; row < NUM_JETS;
       ++row) {  // third particle in event file from SPANET
    for (int col = 0; col < NUM_JETS; ++col) {  // second particle
      for (int z = 0; z < NUM_JETS; ++z) {      // first particle

        if (row == col || row == z || z == col) {
          continue;
        }  // ignore overlapping predictions
  ...
  ANA_MSG_VERBOSE("EventNo: " << eventNumber << ", Max = " << max << ", indx = "
                              << bestrow << "," << bestcol << "," << bestz);

  float* tlpred = this->getOutputs<float>("tl_assignment_probability");

  int bestlb = -1;
  float max_lb = -999;
  for (int i = 0; i < NUM_JETS;
       ++i) {  // loop only over the jets, we dont want to predict the lep entry
    // For our case, we want to prioritise the hadtop prediction over the
    // leptop; so ignore jets in the hadtop prediction

    if (i == bestz || i == bestrow || i == bestcol)
      continue;
    if (tlpred[i] > max_lb) {
      max_lb = tlpred[i];
      bestlb = i;
  ...
  ANA_MSG_VERBOSE("SPANET Down jet = "
                  << bestz << ", up jet = " << bestcol << ", bhad = " << bestrow
                  << ", blep = " << bestlb << " (Njets = " << NUM_JETS << ")");
  ...
  m_regressed_ttbar_m =
      *(this->getOutputs<float>("EVENT/ttbar_invariant_mass"));
}
...
std::vector<float> TopSpaNetTtbarLjetsNu::GetOutputScores() {
  ...
  std::vector<float> scores = {m_hadtop_assignment, m_leptop_assignment,
                               m_hadtop_detection, m_leptop_detection};
...
std::vector<float> TopSpaNetTtbarLjetsNu::GetRegressedValues() {
  ...
  std::vector<float> regressions = {m_regressed_nu_eta, m_regressed_nu_px,
                                    m_regressed_nu_py, m_regressed_nu_pz,
                                    m_regressed_ttbar_m};
