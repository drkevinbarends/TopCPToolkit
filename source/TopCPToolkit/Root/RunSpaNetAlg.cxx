...
RunSpaNetAlg::RunSpaNetAlg(const std::string &name, ISvcLocator *pSvcLocator)
  ...
  declareProperty("topology", m_topology,
                  "SPA-Net topology to use. Choose between: 'TtbarLjetsNu'.");
...
StatusCode RunSpaNetAlg::initialize() {
  ...
  try {
    ...
    ANA_MSG_ERROR("Unrecognized SpaNet topology: "
                  << m_topology << ". Available options: "
                  << SpaNetEnums::printEnumOptions(SpaNetEnums::strToTopology));
  ...
  ANA_CHECK(m_up_idx_decor.initialize(m_systematicsList, m_eventInfoHandle,
                                      SG::AllowEmpty));
  ANA_CHECK(m_lep_top_assignment_decor.initialize(
      m_systematicsList, m_eventInfoHandle, SG::AllowEmpty));
  ANA_CHECK(m_had_top_assignment_decor.initialize(
      m_systematicsList, m_eventInfoHandle, SG::AllowEmpty));
  ANA_CHECK(m_lep_top_detection_decor.initialize(
      m_systematicsList, m_eventInfoHandle, SG::AllowEmpty));
  ANA_CHECK(m_had_top_detection_decor.initialize(
      m_systematicsList, m_eventInfoHandle, SG::AllowEmpty));
  ANA_CHECK(m_reg_nu_eta_decor.initialize(m_systematicsList, m_eventInfoHandle,
                                          SG::AllowEmpty));
  ANA_CHECK(m_reg_nu_px_decor.initialize(m_systematicsList, m_eventInfoHandle,
                                         SG::AllowEmpty));
  ANA_CHECK(m_reg_nu_py_decor.initialize(m_systematicsList, m_eventInfoHandle,
                                         SG::AllowEmpty));
  ANA_CHECK(m_reg_nu_pz_decor.initialize(m_systematicsList, m_eventInfoHandle,
                                         SG::AllowEmpty));
  ANA_CHECK(m_reg_ttbar_m_decor.initialize(m_systematicsList, m_eventInfoHandle,
                                           SG::AllowEmpty));

  ANA_CHECK(m_systematicsList.initialize());

  if (m_topologyEnum == SpaNetEnums::Topology::TtbarLjetsNu) {
    m_spanet_reco = std::unique_ptr<
        top::TopSpaNetTopology>(new top::TopSpaNetTtbarLjetsNu(
        "AsgSpaNetTool" + m_topology,
