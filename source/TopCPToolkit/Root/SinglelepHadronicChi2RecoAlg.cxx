...
SinglelepHadronicChi2RecoAlg::SinglelepHadronicChi2RecoAlg(
    const std::string& name, ISvcLocator* pSvcLocator)
    : EL::AnaAlgorithm(name, pSvcLocator) {
  declareProperty("topMass", m_topMass,
                  "Mass of the top quark for chi^2 (in GeV)");
  declareProperty("maxBJets", m_maxBJets,
                  "Maximum number of b-jets to consider");
  declareProperty("maxLightJets", m_maxLightJets,
                  "Maximum number of light (non-b) jets to consider");
  declareProperty(
      "topResolution", m_topResolution,
      "The sigma parameter for the top quark in the chi^2 (in GeV)");
  declareProperty("wResolution", m_wResolution,
                  "The sigma parameter for the W boson in the chi^2 (in GeV)");
  declareProperty("bTagDecoration", m_bTagDecoration,
                  "Decoration used for b-tagging decision");
  declareProperty("storeFullPermutations", m_storeFullPermutations,
                  "Toggle to store all permutations or just the best one");
}
...
StatusCode SinglelepHadronicChi2RecoAlg::initialize() {
  ...
  ANA_CHECK(m_had_b_idx_decor.initialize(m_systematicsList, m_eventInfoHandle,
                                         SG::AllowEmpty));
  ANA_CHECK(m_light_1_idx_decor.initialize(m_systematicsList, m_eventInfoHandle,
                                           SG::AllowEmpty));
  ANA_CHECK(m_light_2_idx_decor.initialize(m_systematicsList, m_eventInfoHandle,
                                           SG::AllowEmpty));
  ANA_CHECK(m_best_chi2_decor.initialize(m_systematicsList, m_eventInfoHandle,
                                         SG::AllowEmpty));
...
StatusCode SinglelepHadronicChi2RecoAlg::execute_syst(
  ...
  if (bIndices.empty() || lIndices.size() < 2)
    return StatusCode::SUCCESS;
  ...
  do {
    ...
    do {
      auto indices =
          std::make_tuple(bIndices.at(0), lIndices.at(0), lIndices.at(1));
      auto itr = std::find(consideredIndices.begin(), consideredIndices.end(),
                           indices);
      if (itr == consideredIndices.end()) {
        auto indicesReversed =
            std::make_tuple(bIndices.at(0), lIndices.at(1), lIndices.at(0));
      ...
      const double chi2 = this->runSinglePermutation(
          selected_jets, bIndices.at(0), lIndices.at(0), lIndices.at(1));
...
double SinglelepHadronicChi2RecoAlg::chi2(const float recoWmass,
                                          const float recoTopMass) const {
  constexpr double wMass = 80.38;
  const double a = (recoWmass - wMass) * (recoWmass - wMass) /
                   (m_wResolution * m_wResolution);
  const double b = (recoTopMass - m_topMass) * (recoTopMass - m_topMass) /
                   (m_topResolution * m_topResolution);

  return a + b;
}

double SinglelepHadronicChi2RecoAlg::runSinglePermutation(
    const ConstDataVector<xAOD::JetContainer>& jets, const int bIndex,
    const int lIndex1, const int lIndex2) {

  const auto& bjet = jets.at(bIndex);
  const auto& ljet1 = jets.at(lIndex1);
  const auto& ljet2 = jets.at(lIndex2);

  ROOT::Math::PtEtaPhiEVector bHad(bjet->pt() / 1e3, bjet->eta(), bjet->phi(),
                                   bjet->e() / 1e3);

  ROOT::Math::PtEtaPhiEVector l1(ljet1->pt() / 1e3, ljet1->eta(), ljet1->phi(),
                                 ljet1->e() / 1e3);

  ROOT::Math::PtEtaPhiEVector l2(ljet2->pt() / 1e3, ljet2->eta(), ljet2->phi(),
                                 ljet2->e() / 1e3);
