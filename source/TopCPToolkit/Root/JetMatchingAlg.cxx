...
JetMatchingAlg::JetMatchingAlg(const std::string &name,
                               ISvcLocator *pSvcLocator)
    : EL::AnaAlgorithm(name, pSvcLocator) {
  declareProperty("criticalDR", m_criticalDR, "Maximum delta R for matching");
  declareProperty(
      "criticalDR_leptons", m_criticalDR_leptons,
      "Minimum delta R required between reco jet and truth prompt lepton");
  declareProperty("truthIsolationMinPt", m_truthIsolationMinPt,
                  "Minimum truth jet pT to be considered for the dR isolation "
                  "for the truth jets");
  declareProperty("recoIsolationMinPt", m_recoIsolationMinPt,
                  "Minimum reco jet pT to be considered for the dR isolation "
                  "for the reco jets");
}
...
StatusCode JetMatchingAlg::initialize() {
  ...
  ANA_CHECK(
      m_truth_jet_paired_index.initialize(m_systematicsList, m_jetsHandle));
  ANA_CHECK(m_reco_to_reco_jet_closest_dR.initialize(m_systematicsList,
                                                     m_jetsHandle));
  ANA_CHECK(m_truth_to_truth_jet_closest_dR.initialize(m_systematicsList,
                                                       m_jetsHandle));
  ANA_CHECK(m_has_truth_lepton.initialize(m_systematicsList, m_jetsHandle));
  ANA_CHECK(m_overlapping_truth_lepton_pt.initialize(m_systematicsList,
                                                     m_jetsHandle));
...
StatusCode JetMatchingAlg::execute() {
  for (const auto &sys : m_systematicsList.systematicsVector()) {
    ...
    for (const xAOD::Jet *recojet : selected_jets) {
      ...
      float minDR =
          JetMatchingAlg::get_minDR_reco(reco_jet, ireco, selected_jets);
      reco_to_reco_jet_closest_dR.push_back(minDR);
      // find index of truth jet matched to this reco jet (closest within
      // critical deltaR)
      int truth_jet_index =
          JetMatchingAlg::get_matched_truth(reco_jet, *truth_jets);
      matched_truth_jet_indices.push_back(truth_jet_index);
      // for each matched truth jet, find deltaR to its closest truth jet
      if (truth_jet_index == -1)
        truth_to_truth_jet_closest_dR.push_back(
            -1);  // if there was no truth jet matched to this reco
      else {
    ...
    for (std::size_t i = 0; i < matched_truth_jet_indices.size() - 1; i++) {
      for (std::size_t j = i + 1; j < matched_truth_jet_indices.size(); j++) {
        if (matched_truth_jet_indices.at(j) ==
            matched_truth_jet_indices.at(i)) {
    ...
    for (const xAOD::Jet *jet : selected_jets) {
      m_truth_jet_paired_index.set(*jet, matched_truth_jet_indices.at(ijet),
                                   sys);
      m_reco_to_reco_jet_closest_dR.set(
          *jet, reco_to_reco_jet_closest_dR.at(ijet), sys);
      m_truth_to_truth_jet_closest_dR.set(
          *jet, truth_to_truth_jet_closest_dR.at(ijet), sys);
      ...
      m_has_truth_lepton.set(*jet,
                             JetMatchingAlg::find_close_lepton(
                                 reco_jet, *truth_electrons, *truth_muons,
                                 overlapping_truth_lepton_pt),
                             sys);
...
float JetMatchingAlg::get_minDR_reco(
    PtEtaPhiEVector jet1, unsigned int ijet1,
    ConstDataVector<xAOD::JetContainer> selected_jets) {
  float minDR = 9999;
...
int JetMatchingAlg::get_matched_truth(PtEtaPhiEVector reco_jet,
...
bool JetMatchingAlg::find_close_lepton(
  ...
  for (const xAOD::TruthParticle *muon : truth_muons) {
    ...
    if (acc_lepton_fromHadron(*muon) == 0 &&
        DeltaR(reco_jet, TLVmuon) < m_criticalDR_leptons) {
...
float JetMatchingAlg::get_minDR_truth(const xAOD::JetContainer &truth_jets,
                                      int truth_jet_index) {
  float minDR = 9999;
  int itruth1 = 0;
