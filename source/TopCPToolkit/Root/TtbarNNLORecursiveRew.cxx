...
TtbarNNLORecursiveRew::TtbarNNLORecursiveRew(const std::string& name)
    : asg::AsgTool(name), m_systVar{NNLORewEnums::SystVar::NoSyst} {
  ...
  declareProperty("InputDirectory",
                  m_dir = "dev/AnalysisTop/TTbarNNLOReweighter",
                  "Input Directory");
  declareProperty("InputSuffix", m_suffix = "_3iter_2023_LUX", "Input Suffix");
}

TtbarNNLORecursiveRew::~TtbarNNLORecursiveRew() {
  for (auto& entry : m_rewMap)
    entry.second.clear();
  m_rewMap.clear();
  for (auto& entry : m_h_nnlo)
    entry.second.clear();
  m_h_nnlo.clear();
  for (auto& entry : m_h_mc)
    entry.second.clear();
  m_h_mc.clear();
}
...
StatusCode TtbarNNLORecursiveRew::initialize() {
  ...
  m_calibCache.initialize(
      ...
      [this](const CP::SystematicSet& sys, NNLORewEnums::SystVar& systvar) {
        ANA_MSG_WARNING("Mapping for "
                        << sys.name()
                        << "missing, defaulting to no systematic.");
        systvar = NNLORewEnums::SystVar::NoSyst;
        return StatusCode::SUCCESS;
      });

  // loop over all systematics and map the corresponding histograms and
  // reweighting recipes
  for (const auto& pair : NNLORewEnums::strToSystVar) {
    ...
    if (m_type == NNLORewEnums::RewType::Rew2D) {
      ...
      if (thisSystVar >= NNLORewEnums::SystVar::Inv3D1 &&
          thisSystVar <= NNLORewEnums::SystVar::Inv3D5) {
    ...
    // fill the map of reweightings with the recipe corresponding to this
    // systematic
    ...
    switch (thisSystVar) {
      ...
      case NNLORewEnums::SystVar::QEDPDFChoice:
        if (suffix.find("_LUX") != std::string::npos)
          suffix.erase(suffix.find("_LUX"), 4);
        else
          suffix.append("_LUX");
        break;
      case NNLORewEnums::SystVar::Inv2D:
        if (suffix.find("_LUX") != std::string::npos)
          suffix.replace(suffix.find("_LUX"), 4, "_inv_LUX");
        else
          suffix.append("_inv");
        break;
      case NNLORewEnums::SystVar::Inv3D1:
        if (suffix.find("_LUX") != std::string::npos)
          suffix.replace(suffix.find("_LUX"), 4, "_inv1_LUX");
        else
          suffix.append("_inv1");
        break;
      case NNLORewEnums::SystVar::Inv3D2:
        if (suffix.find("_LUX") != std::string::npos)
          suffix.replace(suffix.find("_LUX"), 4, "_inv2_LUX");
        else
          suffix.append("_inv2");
        break;
      case NNLORewEnums::SystVar::Inv3D3:
        if (suffix.find("_LUX") != std::string::npos)
          suffix.replace(suffix.find("_LUX"), 4, "_inv3_LUX");
        else
          suffix.append("_inv3");
        break;
      case NNLORewEnums::SystVar::Inv3D4:
        if (suffix.find("_LUX") != std::string::npos)
          suffix.replace(suffix.find("_LUX"), 4, "_inv4_LUX");
        else
          suffix.append("_inv4");
        break;
      case NNLORewEnums::SystVar::Inv3D5:
        if (suffix.find("_LUX") != std::string::npos)
          suffix.replace(suffix.find("_LUX"), 4, "_inv5_LUX");
        else
          suffix.append("_inv5");
        break;
      default:
    ...
    if (((thisSystVar == NNLORewEnums::SystVar::ScaleUpTTbarPt ||
          thisSystVar == NNLORewEnums::SystVar::ScaleDownTTbarPt) &&
         m_type == NNLORewEnums::RewType::Rew2D) ||
        (thisSystVar == NNLORewEnums::SystVar::Inv2D &&
         m_type == NNLORewEnums::RewType::Rew3D) ||
        ((thisSystVar >= NNLORewEnums::SystVar::Inv3D1 &&
          thisSystVar <= NNLORewEnums::SystVar::Inv3D5) &&
         m_type == NNLORewEnums::RewType::Rew2D)) {
      ANA_MSG_INFO("Unsupported systematic variation "
                   << NNLORewEnums::systVarToStr(thisSystVar)
                   << " for reweighting of type "
                   << NNLORewEnums::rewTypeToStr(m_type)
                   << ". Will be ignored.");
    ...
    std::unique_ptr<TFile> f(TFile::Open(
        (m_dir + "/RecursiveReweighting__" + smpStr + sysStr + suffix + ".root")
            .c_str()));
    if (!f)
      ANA_MSG_ERROR("Cannot open the file! Aborting.");
    else
    ...
    for (unsigned int i_rew = 0; i_rew < m_rewMap[thisSystVar].size();
         i_rew++) {
      // NNLO predictions
      histname = "h_" + m_rewMap[thisSystVar][i_rew];
      auto tmp_nnlo = dynamic_cast<TH1*>(f->Get(histname.c_str()));
      if (!tmp_nnlo)
        ANA_MSG_ERROR("Could not retrieve NNLO histogram " << histname
                                                           << "! Aborting.");
      ...
      // MC predictions
      if (i_rew == 0)
        histname = "h_" + m_rewMap[thisSystVar][i_rew] + "_" + smpStr;
      else
        histname = "h_" + m_rewMap[thisSystVar][i_rew] + "_" + smpStr +
                   "_rew_" + std::to_string(i_rew - 1);
      auto tmp_mc = dynamic_cast<TH1*>(f->Get(histname.c_str()));
      if (!tmp_mc)
        ANA_MSG_ERROR("Could not retrieve MC histogram " << histname
                                                         << "! Aborting.");
    ...
    // fill the map of reweightings with the histograms corresponding to this
    // systematic
...
StatusCode TtbarNNLORecursiveRew::buildCachedSystematics() {
  ...
  if (m_sampleId != NNLORewEnums::SampleId::Nominal) {
    ANA_MSG_INFO("Running on alternative sample of type "
                 << NNLORewEnums::sampleIdToStr(m_sampleId)
                 << " for which there are no systematic.");
    return StatusCode::SUCCESS;
  }
  ...
  for (const auto& pair : NNLORewEnums::strToSystVar) {
    ...
    // ignore nominal
    if (thisSystVar == NNLORewEnums::SystVar::NoSyst)
      continue;

    // the same additional edge cases that we warned about in the initialize()
    // method
    if (((thisSystVar == NNLORewEnums::SystVar::ScaleUpTTbarM ||
          thisSystVar == NNLORewEnums::SystVar::ScaleDownTTbarM) &&
         m_type == NNLORewEnums::RewType::Rew2D) ||
        (thisSystVar == NNLORewEnums::SystVar::Inv2D &&
         m_type == NNLORewEnums::RewType::Rew3D) ||
        ((thisSystVar >= NNLORewEnums::SystVar::Inv3D1 &&
    ...
    if (sysname.find("Up") != std::string::npos) {
      ...
      ANA_CHECK(m_calibCache.add(
          CP::SystematicVariation((sysPrefix + sysname).c_str(), 1),
          thisSystVar));
      ...
      ANA_CHECK(m_calibCache.add(
          CP::SystematicVariation((sysPrefix + sysname).c_str()), thisSystVar));
    }
  }
...
void TtbarNNLORecursiveRew::SetInverted3D(std::vector<std::string>& rewList,
                                          int i) {
  rewList.clear();
  if (i == 1) {
    rewList.emplace_back("TTbarM");
    ...
    ANA_MSG_ERROR("Invalid argument for SetInverted3D: "
                  << i << " (valid options are 1, 2, 3, 4, 5)");
  }
}
...
double TtbarNNLORecursiveRew::Reweight(double x, int idx) {
  ...
  if (bin == 0 || bin > m_h_nnlo[m_systVar][idx]->GetNbinsX())
    return 1.;
...
double TtbarNNLORecursiveRew::GetSysWeight(const CP::SystematicSet& sys,
                                           double Top_pt, double ATop_pt,
                                           double TTbar_M, double TTbar_Pt) {
  ...
  for (unsigned int i_rew = 0; i_rew < m_rewMap[m_systVar].size(); i_rew++) {
    if (m_rewMap[m_systVar][i_rew] == "TopPt")
      result *= sqrt(Reweight(Top_pt, i_rew) * Reweight(ATop_pt, i_rew));
    else if (m_rewMap[m_systVar][i_rew] == "TTbarPt")
      result *= Reweight(TTbar_Pt, i_rew);
    else if (m_rewMap[m_systVar][i_rew] == "TTbarM")
      result *= Reweight(TTbar_M, i_rew);
  }
  if (m_systVar == NNLORewEnums::SystVar::FOvsPSTopPt) {
    double weight_top =
        Top_pt > 0 ? 9.78044e-01 + sqrt(4.35553e-06 * Top_pt / 1e3) : 1.;
    double weight_atop =
        ATop_pt > 0 ? 9.78044e-01 + sqrt(4.35553e-06 * ATop_pt / 1e3) : 1.;
    result *= sqrt(weight_top * weight_atop);
  } else if (m_systVar == NNLORewEnums::SystVar::FOvsPSTTbarM) {
    result *=
        TTbar_M > 0 ? 9.62236e-01 + sqrt(2.78789e-06 * TTbar_M / 1e3) : 1.;
  }
  return result;
}
