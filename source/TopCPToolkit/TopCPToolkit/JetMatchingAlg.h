...
class JetMatchingAlg final : public EL::AnaAlgorithm {
 ...
 private:
  ...
  CP::SysReadSelectionHandle m_selection{
      this, "eventSelection", "",
      "Name of the selection on which this JetMatching instance is allowed to "
      "run"};

  // output decorations
  CP::SysWriteDecorHandle<int> m_truth_jet_paired_index{
      this, "truth_jet_paired_index", "truth_jet_paired_index_%SYS%",
      "Index of the matched true jet"};
  CP::SysWriteDecorHandle<float> m_reco_to_reco_jet_closest_dR{
      this, "reco_to_reco_jet_closest_dR", "reco_to_reco_jet_closest_dR_%SYS%",
      "dR of the closest reco jet"};
  CP::SysWriteDecorHandle<float> m_truth_to_truth_jet_closest_dR{
      this, "truth_to_truth_jet_closest_dR",
      "truth_to_truth_jet_closest_dR_%SYS%",
      "dR between the matched truth jet and its closest truth jet"};
  CP::SysWriteDecorHandle<bool> m_has_truth_lepton{
      this, "has_truth_lepton", "has_truth_lepton_%SYS%",
      "reco jet has truth lepton at dR < 0.4"};

  CP::SysWriteDecorHandle<float> m_overlapping_truth_lepton_pt{
      this, "overlapping_truth_lepton_pt", "overlapping_truth_lepton_pt_%SYS%",
      "pt of the overlapping truth lepton"};
  ...
  float get_minDR_reco(PtEtaPhiEVector jet1, unsigned int ijet1,
                       ConstDataVector<xAOD::JetContainer> selected_jets);
  float get_minDR_truth(const xAOD::JetContainer &truth_jets,
                        int truth_jet_index);
  int get_matched_truth(PtEtaPhiEVector reco_jet,
                        const xAOD::JetContainer &truth_jets);
  bool find_close_lepton(PtEtaPhiEVector &reco_jet,
