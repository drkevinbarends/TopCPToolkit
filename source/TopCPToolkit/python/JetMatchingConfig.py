...
class JetMatchingConfig(ConfigBlock):
    ...
    def __init__(self):
        ...
        self.addOption("truthIsolationMinPt", -1.0, type=float)
        self.addOption("recoIsolationMinPt", -1.0, type=float)
    ...
    def makeAlgs(self, config):
        ...
        config.addOutputVar(
            self.jets.split(".")[0],
            alg.truth_jet_paired_index,
            "truth_jet_paired_index",
            noSys=True,
        )
        config.addOutputVar(
            self.jets.split(".")[0],
            alg.reco_to_reco_jet_closest_dR,
            "reco_to_reco_jet_closest_dR",
        )
        config.addOutputVar(
            self.jets.split(".")[0],
            alg.truth_to_truth_jet_closest_dR,
            "truth_to_truth_jet_closest_dR",
            noSys=True,
        )
        config.addOutputVar(
            self.jets.split(".")[0],
            alg.has_truth_lepton,
            "has_truth_lepton",
            noSys=True,
        )
        config.addOutputVar(
            self.jets.split(".")[0],
            alg.overlapping_truth_lepton_pt,
