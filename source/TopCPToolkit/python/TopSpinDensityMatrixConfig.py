...
class TopSpinDensityMatrixConfig(ConfigBlock):
    ...
    def makeAlgs(self, config):
        if not (self.top and self.tbar and self.top_decay and self.tbar_decay):
            raise ValueError(
                "You must specify the names of the 4-vectors for top/tbar and their spin analysers!"
            )

        alg = config.createAlgorithm(
            "top::TopSpinDensityMatrixAlg", "TopSpinDensityMatrix" + self.setup
        )
        # event selection
        ...
        for observable in observables:
            config.addOutputVar(
                "EventInfo", observable, "spin_" + observable.split("_%SYS%")[0]
            )
