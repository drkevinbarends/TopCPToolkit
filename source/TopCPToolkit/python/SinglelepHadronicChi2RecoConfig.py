...
class SinglelepHadronicChi2RecoConfig(ConfigBlock):
    ...
    def __init__(self):
        ...
        self.addOption("topResolution", 35.0, type=float)
        self.addOption("wResolution", 25.0, type=float)
    ...
    def makeAlgs(self, config):
        alg = config.createAlgorithm(
            ...
            "SinglelepHadronicChi2RecoAlg_" + self.postfix,
        )
        ...
        config.addOutputVar(
            "EventInfo", alg.hadronic_b_index, "chi2_had_b_index_" + self.postfix
        )
        config.addOutputVar(
            "EventInfo", alg.light_1_index, "chi2_light_1_index_" + self.postfix
        )
        config.addOutputVar(
            "EventInfo", alg.light_2_index, "chi2_light_2_index_" + self.postfix
        )
        config.addOutputVar("EventInfo", alg.best_chi2, "best_chi2_" + self.postfix)
