...
class TtbarNNLORecursiveRewConfig(ConfigBlock):
    ...
    def makeAlgs(self, config):
        ...
        if config.geometry() == LHCPeriod.Run3:
            print(
                "Run 3 NNLO ttbar reweighting is not supported yet! skipping the algorithm."
            )
            return
        ...
        if self.sampleID == "AutoConfig":
            for key, value in map_sampleID.items():
                ...
                print(
                    f"Sample with DSID {config.dsid()} is not supported for ttbar NNLO reweighting, skipping the algorithm."
                )
                return

        alg = config.createAlgorithm(
            "top::RunTtbarNNLORecursiveRewAlg",
            "RunTtbarNNLORecursiveRewAlg" + self.reweightType,
        )
        ...
        config.addOutputVar(
            "EventInfo", alg.NNLO_weight, "weight_NNLO_" + self.reweightType
        )
