...
"""
    ...
    print("")
    print(
        f"Hi there. It looks like you are running {orange_code}a configuration that is only meant as an example{reset_code}."
    )
    print("")
    print(
        "This could go wrong fast, if you are using it as a starting point for your own analysis without being careful!"
    )
    print("")
    print(
        f"We suggest you head over to {blue_code}https://topcptoolkit.docs.cern.ch/{reset_code}\nand hit the {blue_code}Getting started{reset_code} and {blue_code}Tutorials{reset_code} pages."
    )
    print("")
    print(
        "If you are just running this config as an example, no worries :) Better safe than sorry."
    )
