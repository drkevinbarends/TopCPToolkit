...
_campaigns_R2 = [Campaign.MC20a, Campaign.MC20d, Campaign.MC20e]
...
def populate_config_flags(flags):
    ...
    if len(flags.Input.RunNumbers) != 1:
        print(
            "WARNING (metaConfig.populate_config_flags): FileMetaData reports RunNumber list "
...
def get_campaign_fallback(flags):
    ...
    if amiTags == "":
        print(
            "WARNING (metaConfig.get_campaign): AMITag entry in FileMetaData "
    ...
    for cmp, tagsList in _campaigns_AMITag.items():
        for tag in tagsList:
            if tag in amiTags:
                return cmp
    raise Exception(
        f"AMITag {amiTags} in FileMetaData does not correspond to any implemented campaign"
    )
...
def get_data_year(flags):
    ...
    if flags.Input.RunNumberAsInt == 0:
        print(
            "ERROR (metaConfig.get_data_year): runNumber == 0, we cannot determine data year reliably."
        )
    for year, runRange in _years_runNumbers.items():
        if (
            flags.Input.RunNumberAsInt >= runRange[0]
            and flags.Input.RunNumberAsInt < runRange[1]
        ):
            return year
    print(
        f"WARNING (metaConfig.get_data_year): runNumber {flags.Input.RunNumberAsInt} "
...
def isPhysLite(flags):
    ...
    if flags.Input.ProcessingTags is not None:
        ...
        print(
            "WARNING Could not find any information about the sample being PHYSLITE "
...
def isTRUTH(flags):
    ...
    if flags.Input.ProcessingTags is not None:
        return (
            "StreamDAOD_TRUTH1" in flags.Input.ProcessingTags
            or "StreamDAOD_TRUTH3" in flags.Input.ProcessingTags
        )
    else:
        print(
            "WARNING Could not find any information about the sample being TRUTHx "
            "in the metadata. Will assume that it was regular PHYS."
        )
    return False
