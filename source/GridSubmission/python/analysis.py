...
import GridSubmission.ami as ami
...
def cacheAmi(ptaglist):
    try:
        ...
        print(
            logger.OKBLUE
            + "Using cached yields - to force me to query AMI delete amiyields.pickle"
            + logger.ENDC
        )
        ...
        if ioex.errno == 2:
            ...
            for ptag in ptaglist:
                ...
                queryAmi(
                    amiSamples,
                    "data16_13TeV.period%physics_Main.PhysCont.DAOD_TOPQ%" + ptag,
                )
                queryAmi(
                    amiSamples,
                    "data15_13TeV.period%physics_Main.PhysCont.DAOD_TOPQ%" + ptag,
                )
            ...
            if len(amiSamples) == 0:
                print(
                    logger.FAIL
                    + "Something went wrong when trying to get the data from ami"
                    + logger.ENDC
                )
                sys.exit(1)
            else:
