...
def compareTrees(ref_file, new_file):
    ...
    if miss_ref or miss_new:
        logger.warning(
            f"{orange_code}Warning: the number of TTrees differs between the new file and the reference file.{reset_code}"
        )
        if miss_ref:
            logger.warning(
                f"{orange_code}  --> the following are missing from the reference file:{reset_code} {miss_ref}"
            )
        if miss_new:
            logger.warning(
                f"{orange_code}  --> the following are missing from the new file:{reset_code} {miss_new}"
            )
    if common:
        logger.info(
            f"{blue_code}The following TTrees are common to both the reference and new files:{reset_code} {common}"
        )
...
def compareHistograms(ref_file, new_file):
    ...
    if miss_ref or miss_new:
        logger.warning(
            f"{orange_code}Warning: the number of TH1 histograms differs between the new file and the reference file.{reset_code}"
        )
        if miss_ref:
            logger.warning(
                f"{orange_code}  --> the following are missing from the reference file:{reset_code} {miss_ref}"
    ...
    if common:
        logger.info(
            f"{blue_code}There are {len(common)} TH1 histograms common to both the reference and new files.{reset_code}"
        )
    failure = bool(miss_ref or miss_new)
...
def compareSystematics(ref_file, new_file):
    ...
    ref_sys = [
        ref_histo.GetXaxis().GetBinLabel(i + 1)
        for i in range(ref_histo.GetXaxis().GetNbins())
    ]
    new_sys = [
        new_histo.GetXaxis().GetBinLabel(i + 1)
        for i in range(new_histo.GetXaxis().GetNbins())
    ]
    common, miss_ref, miss_new = compareLists(ref_sys, new_sys)
    if miss_ref or miss_new:
        logger.warning(
            f"{orange_code}Warning: the number of systematics differs between the new file and the reference file.{reset_code}"
        )
        if miss_ref:
            logger.warning(
                f"{orange_code}  --> the following are missing from the reference file:{reset_code} {miss_ref}"
    ...
    if common:
        logger.info(
            f"{blue_code}There are {len(common)} systematics common to both the reference and new files.{reset_code}"
        )
    failure = bool(miss_ref or miss_new)
...
def compareBranches(key, ref_file, new_file):
    ...
    if miss_ref or miss_new:
        logger.warning(
            f"{orange_code}Warning: the number of TBranches differs between the new file and the reference file for TTree {key}.{reset_code}"
        )
        if miss_ref:
            logger.warning(
                f"{orange_code}  --> the following are missing from the reference file:{reset_code} {miss_ref}"
    ...
    if common:
        logger.info(
            f"{blue_code}There are {len(common)} TBranches common to both the reference and new files for TTree {key}.{reset_code}"
        )
    failure = bool(miss_ref or miss_new)
...
def compareEvents(key, branches, ref_file, new_file):
    ...
    if ref_events != new_events:
        logger.warning(
            f"{orange_code}Warning: the number of events differs between the new file and the reference file for TTree {key}.{reset_code}"
        )
    ...
    for event_idx in range(ref_events):
        ...
        new_tree.GetEntry(
            new_tree.GetEntryNumberWithIndex(getattr(ref_tree, "eventNumber"))
        )
        if getattr(ref_tree, "eventNumber") != getattr(new_tree, "eventNumber"):
            logger.warning(
                f"{orange_code}  Entry with eventNumber {getattr(ref_tree,'eventNumber')} doesn't exist in the new file. Skipping.{reset_code}"
            )
            continue
        for branch in branches:
            ...
            if ref_value != new_value:
                ...
                if (
                    "spanet_had_top_score" in branch
                    or "spanet_had_top_assignment" in branch
                ):
                    if math.isclose(ref_value, new_value, rel_tol=1e-5):
                        continue
                logger.debug(
                    f"{orange_code}Warning: Branch {branch} differs for event {getattr(ref_tree,'eventNumber')}:{reset_code}"
                )
    ...
    if faulty_branches:
        ...
        logger.warning(
            f"{orange_code}The following branches didn't match between the new file and the reference file (consult the log for details):{reset_code} {faulty_branches}"
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        logger.error(
            f"{red_code}The script compare_samples.py requires 2 arguments!{reset_code}"
        )
        logger.error(
            "Please run it as : python3 compare_samples.py <reference.root> <new.root>"
        )
        exit(1)
    ...
    if global_failure:
        logger.error(
            f"{red_code}At least one of the tests failed! Please check the logs carefully.{reset_code}"
        )
        exit(1)
    else:
