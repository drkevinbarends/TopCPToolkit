...
TutorialAlg::TutorialAlg(const std::string &name, ISvcLocator *pSvcLocator)
...
StatusCode TutorialAlg::initialize() {
  ...
  ANA_CHECK(m_preselection.initialize(m_systematicsList, m_eventInfoHandle,
                                      SG::AllowEmpty));
...
StatusCode TutorialAlg::execute() {
  ...
  for (const auto &sys : m_systematicsList.systematicsVector()) {
    ...
    float mtw =
        std::sqrt(2 * lepton.Pt() * et_miss * (1 - std::cos(dphi_lep_met)));
